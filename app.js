...
db.collection('Vocabulary').get().then(snap => {
  ...
  for(let i = 0; i < size; i++){
    ...
    class="btn btn-primary" style = "background-color : ${buttonColor} ;border-color: ${buttonColor} ;margin : 2px">${i+1}</button>
    `
  }
});
  ...
  for(let i = 0; i < 20; i++){
      document.getElementById("addVoca").innerHTML += `
            <div class="input-group">
              <button class="btn btn-primary" style ="margin : 2px">${i+1}. ${randomArrayDefs[i]}</button>
              <div style = "display: inline-block;">
                <input type="text" id = "Words${i+1}" style = " margin : 2px ;width = 50px"class="form-control">
              </div>
              <button id = "finish${i+1}" style="display: none" class="btn btn-primary">${resultWords[i]}</button>
            </div>
...
function checkWrdsDefs(){
  ...
  document.getElementById("base-score-label").innerHTML = `${score} / ${SCORE_LIMIT}` ;
...
async function getDefinitions(){
  var docRef = db.collection("Vocabulary").where("Part", "==", "" +document.getElementById("count-part").value )
...
function addVoca(){
  $(document).ready(function(){
    count_part = parseInt(document.getElementById("count-part").value) + 1;
      db.collection("Vocabulary").doc("Part" + count_part ).set({
...
function SetCircleDasharrayScore(score) {
  const circleDasharray = `${(
    (1 - score/SCORE_LIMIT) * FULL_DASH_ARRAY
  ).toFixed(0)} 283`;
  document
